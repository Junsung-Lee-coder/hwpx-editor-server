...
def reconcile_command_journal(session_root: Path, command_id: str) -> dict[str, Any]:
    """Mark a completed timed-out command as consumed, atomically."""
...
class LocalCliLiveSession:
    ...
    def _command_record(self, command: _LiveCommand) -> dict[str, Any]:
        ...
        if command.result is not None and command.state in {'succeeded', 'completed_after_timeout'}:
            record['result'] = _bounded_command_result(command.result)
    ...
    def reconcile_command(self, command_id: str) -> dict[str, Any]:
        ...
        with self._state_lock:
            ...
            if command is None:
                return reconcile_command_journal(self.session_root, command_id)
        ...
        *,
        ...
        with self._state_lock:
            ...
            error = LocalCliRuntimeTimeoutError(
                f'{message} Session terminalized; command outcome is reconcilable by command_id={command_id}.',
                command_id=command_id,
                command_state=state,
            )
            ...
            # A timed-out close sentinel may still be queued rather than
            # running.  Keep that one command available so the worker can
            # perform its native/COM teardown; ordinary pending work is
            # cancelled and cannot race the terminalized session.
            preserve_command = (
                command
                if command is not None and command.name == '__close__' and not enqueue_close
                else None
            )
            self._cancel_pending_commands(
                'The live local CLI session terminalized after a command timeout.',
                preserve_command=preserve_command,
            )
            if enqueue_close:
...
class LocalCliRuntimeManager:
    ...
    def reconcile_command(
        ...
        *,
        ...
        return reconcile_command_journal(root, command_id)
